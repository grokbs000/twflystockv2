...
- [x] AppLayout 側邊欄導航（優質股雷達、觀察清單、篩選歷史、篩選設定）
- [x] 首頁儀表板：統計摘要、最新優質股清單、條件篩選 Tab
...
- [x] 前端：通知卡片顯示篩選結果摘要（找到幾支優質股、股票清單）
...
- [x] 前端儀表板加入即時進度條（已掃描 X/N 支、百分比、發現優質股數量）
...
- [x] Dashboard 計數器橫幅下方即時顯示最新找到的優質股（最多 3 張，新的在前）

## 緊急修復
- [x] 修復「優質股篩選啟動中」卡住不動的問題（加入 SSE ping 心跳、進度更新頻率提高至每 10 支）
